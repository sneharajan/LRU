...
UnpinBuffer(volatile BufferDesc *buf, bool fixOwner, bool normalAccess)
{
	...
	if (PrivateRefCount[b] == 0)
	{
      ...
      //DBProject 3 changes:In order to remove the dead pages, the refcount needs to be checked. The usage count is not required since its an LRU implementation          
     ...
     //DBProject 3 changes: the free buf needs to be appended at the end of the linked list at all times in the LRU implementation
